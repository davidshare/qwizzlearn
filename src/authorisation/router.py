...
@ authorisation_router.delete("/roles/permissions/{role_id}", dependencies=[Depends(AccessTokenBearer())], status_code=status.HTTP_200_OK)
async def revoke_user_permissions(role_id: int, permission_ids: List[int], session: AsyncSession = Depends(get_session)):
